function [ reported_objective,reported_error ] = Bayes_processFopt_normal_dataset( dataset,test_dataset,Label,best_params,wanted_error ) %train,test,validate
%UNTITLED6 Summary of this function goes here
%   Detailed explanation goes here
tic
for c = 1 : 1
[ group_vote_counts_test,mean_mean_acc,mean_std_acc ,yfit_unknown_test,validate_counts,train_counts,test_labels] = Bayes_process_inner_nomal_dataset(dataset,test_dataset,Label,best_params.prct,best_params.learners,best_params.predictors,best_params.min_leaf);

%% Bayes
          for cc = 1 : length(Label)% length(New_Labels)
            for i = 1 : length(Label)% length(New_Labels)
                   temp = group_vote_counts_test{cc}(test_dataset(:,32) == Label(i),:);
                   if(~isempty(temp))
                     if(Label(i) == Label(cc))
                        temp_yfit = yfit_unknown_test{cc};
                        tteemmpp1 = new_test_data(test_dataset(:,32) == Label(cc),:);
                     else
