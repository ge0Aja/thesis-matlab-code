function [ reported_error,reported_constraint ] = Process_error( params,group_vote_counts_test,yfit_unknown_test,yfit_known_test,test_zeros,mean_mean_acc,mean_std_acc,max_fpr_rate )
...
for c = 1 : 8
    for i = 1 : 8 
           temp = group_vote_counts_test{c}(test_zeros(:,32) == i,:);
             ...
             unique_flows_stats_eff = unique_flows(unique_flows_counts > params.min_flow_length); %params.min_flow_length
             unique_flows_counts = unique_flows_counts( unique_flows_counts > params.min_flow_length); %params.min_flow_length
...
%                  end
                 ...
                 lambda_i_j_history = params.lambda_i_j_history;% params.lambda_i_j_history;
                 lambda_i_j_future = params.lambda_i_j_future;% params.lambda_i_j_future;
...
%                             sample_class_index(k) = find (temp_most_appearing_class == max(temp_most_appearing_class),1);
                            ...
                            if(temp_most_appearing_class_ratios(top_ratio) > params.confidence_ratio) % params.confidence_ratio
...
% reported_error = 1 - mean(tq(1:2));
% reported_constraint = mean(mean_mean_fpr) - max_fpr_rate;

 reported_error  = mean(mean_mean_fpr(1:2));
 reported_constraint = (1-mean(tq(1:2))) - 0.05;
end
