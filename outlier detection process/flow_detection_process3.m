...
for c = 1 : 8
    for i = 1 : 8 
           temp = group_vote_counts_test{c}(test_zeros(:,32) == i,:);
             ...
             unique_flows_stats_eff = unique_flows(unique_flows_counts > 10); %params.min_flow_length
             unique_flows_counts = unique_flows_counts( unique_flows_counts > 10); %params.min_flow_length
             for j =  1 : size(unique_flows_stats_eff,1)
                 ...
                 w = 200;% params.w;
                 alpha = 0.48887 ;% params.alpha;
%                  beta = 0.7;%params.beta;
                 lambda_i_j_history = 0.50335;% params.lambda_i_j_history;
                 lambda_i_j_future = 0.65453;% params.lambda_i_j_future;
...
%                             sample_class_index(k) = find (temp_most_appearing_class == max(temp_most_appearing_class),1);
                            ...
                            if(temp_most_appearing_class_ratios(top_ratio) > 0.75) % params.confidence_ratio
