function [ eval_res_tq,eval_res_tq1,eval_res,mean_mean_temp_mean,mean_mean_eval_res_tq,Results_Decisions ] = Bayes_processFvSingleUser( train,test,validate,Label,best_params)
...
[ group_vote_counts_test,new_test_data,mean_mean_acc,mean_std_acc ,yfit_unknown_test,yfit_known_test,~,~,~] = Bayes_process_inner_single_user(train,test,validate,Label,best_params.prct,best_params.learners,best_params.predictors,best_params.min_leaf);

%% Bayes
        for cc = 1 : length(Label)% length(Label)
            for i = 1 : length(Label)% length(Label)
                   ...
                   if(~isempty(temp))
                     ...
                     for j =  1 : size(unique_flows_stats_eff,1)
                         ...
                         for k = 1: unique_flows_counts(j)
                            ...
                            temp_dzscore(k,:) = (unique_flow(k,Label) - mean_mean_acc) ./ mean_std_acc;
                                ...
                                if(k ~= 1)
                                    ...
                                    % start future 
                                     if(k < unique_flows_counts(j))
                                        ...
                                        temp_dzscore_fututre= zeros(unique_flows_counts(j),length(Label));
                                        temp_pvalues_fututre = zeros(unique_flows_counts(j),length(Label));
                                        counter = 1;
                                        for l = k+1  : temp_w_future
                                            temp_dzscore_fututre(counter,:) = (unique_flow(l,1:length(Label)) - mean_mean_acc) ./ mean_std_acc;
...
for c = 1 : 1
   for i = 1 : length(Label)%length(Label) 
        if(~isempty(Results_Decisions{c}{i}))
            for j = 1 : length(Label)%length(Label)
...
for c = 1 : 1
    for i = 1 : length(Label)%length(Label)
        temp_mean(c,:)  =mean(eval_res{c}{1}(Label ~= Label(i),:)); %length(Label)
