function [ reported_objective,reported_error ] = Bayes_processFoptv2( Data_Sets22,Label,Extra_Apps,Extra_Labels,best_params,Game_G,wanted_error,Brows)
...
for c = 1 : length(Data_Sets22)
       for i = 1 : length(Results_Decisions{c})%length(New_Labels) 
            for j = 1 : length(Results_Decisions{c}{i})%length(New_Labels)
...
%     mean_mean_fpr(i) = temp_mean(i);
   ...
   mean_mean_fpr = mean (mean_mean_temp_mean(2:end));
