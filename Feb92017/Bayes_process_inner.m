...
for l = 1 : length(Label)
    for i = 1 : length(Label) 
           temp = group_vote_counts_test{l}(new_test_data_balanced_10(:,32) == Label(i),:);
           if(~isempty(temp))
             if(i == l)
                temp_yfit = yfit_unknown_test{l};
                tteemmpp1 = new_test_data_balanced_10(new_test_data_balanced_10(:,32) == Label(l),:);
             else
                 tteemmpp1 = new_test_data_balanced_10(new_test_data_balanced_10(:,32) ~= Label(l),:);
                 tteemmpp2 = (tteemmpp1(:,32) == Label(i));
                 temp_yfit = yfit_known_test{l}(tteemmpp2);
             end
...
%                             sample_class_index(k) = find (temp_most_appearing_class == max(temp_most_appearing_class),1);
                 ...
                 Results_Decisions{c}{l}{i}{j} = sample_class_index';
             end
         end
    end
end
...
for i = 1 : length(Label) 
    for j = 1 :  length(Label)
        ...
        if(i ~= j)      
           ttoo2 = sum(cat(1,Results_Decisions{c}{i}{j}{:}) == 0);   
           ttoo3 = sum(cat(1,Results_Decisions{c}{i}{j}{:}) == Label(j));

           tq2(i,j) = ttoo2 ./ ttpp2;
           tq3(i,j) = ttoo3 ./ ttpp2;
        else
            ttoo1 = sum(cat(1,Results_Decisions{c}{i}{j}{:}) ~= 0);
            ttoo = sum(cat(1,Results_Decisions{c}{i}{j}{:}) == 0);
            tq(i) = ttoo ./ ttpp2;
            tq1(i) = ttoo1 ./ ttpp2;
        end
    end  
