function [ reported_err ] = optimizeKFold( Data_Sets,game_321_2_2,Label,params )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

parfor c = 1 : length(Data_Sets) 
train = Data_Sets{c}{1};
test = Data_Sets{c}{2};
    ...
    min_count_exp_w = min(train_counts([1:length(Label)]  & [1:length(Label)] ~=2 )); % & [1:6] ~= l
...
%             [tpr{c}(pp,:),fpr{c}(pp,:),f1{c}(pp,:)] = CalcTRPFPR(conf);
            ...
            [~,ParRes22{c}{pp}] = parRandForesttest( new_train_data_balanced,new_test_data_balanced_10,params.Trees,params.Predictors,params.minLS);
...
for i =1 : length(ParRes22)
        for j = 1 : length(ParRes22{i})
            f1{i}(j,:) = ParRes22{i}{j}{4};  
        end
end
...
for i = 1: 9
    ...
    reported_err =  1 - mean( mean(mean_f1) );
