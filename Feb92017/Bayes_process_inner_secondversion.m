function [ group_vote_counts_test,new_test_data_balanced_10,mean_mean_acc,mean_std_acc ,yfit_unknown_test,yfit_known_test,Extended_Label ] = Bayes_process_inner_secondversion( c,Data_Sets22,Label,Extra_Apps,Extra_Labels,Game_G,Brows,minls,predictors)
...
%         end
            ...
            tr_b = [];
            te_b = [];
%           if(~ isempty(Extra_Apps))  
            while(isempty(val_g) && isempty(tr_g) && isempty(te_g))
            [~,val_g] = getTrain_Test( Game_G,22,[2]);
            [tr_g,te_g] = getTrain_Test( Game_G,22,[2]);
            end
            
            while(isempty(val_b) && isempty(te_b) && isempty(te_b))
            [~,val_b] = getTrain_Test( Brows,20,[1]);
            [tr_b,te_b] = getTrain_Test( Brows,20,[1]);
            end
%           end
          if(~isempty(Extra_Apps))
...
%               
                  ...
                  te{ll} = temp(test_ind,:);
                  temp(test_ind,:) = [];
                  val_ind = randperm(size(temp,1),round(size(temp,1) * 0.1));
                  val{ll} =  temp(val_ind,:);
                  temp(val_ind,:) = [];
                  tr{ll} = temp;
             end
             
             all_val = cat(1,val{:},val_g,val_b);
             all_tr = cat(1,tr{:},tr_g,tr_b);
             all_te = cat(1,te{:},te_g,te_b);
          
             Extended_Label = sort(horzcat(Label,Extra_Labels));
          else
              all_val = cat(1,val_g,val_b);
              all_tr = cat(1,tr_g,tr_b);
              all_te = cat(1,te_g,te_b);
             ...
             for ll = 1 : length(Extended_Label)
                if(sum(Label == Extended_Label(ll)) > 0 )
                    if(Extended_Label(ll) ~= 2) %&& Extended_Label(ll) ~= 1
...
%                  
            ...
            parfor l = 1 : length(Extended_Label)
                [Classifier{l},~] = parRandForesttest( new_train_data_balanced(new_train_data_balanced(:,32) ~= Extended_Label(l),:),new_validate_data_balanced_10,30,predictors,minls);
            end     
%             clear pred_known
%             clear pred_unknown
            parfor l = 1 : length(Extended_Label)
