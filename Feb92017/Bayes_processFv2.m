function [ eval_res_tq,eval_res_tq1,eval_res,mean_mean_temp_mean,mean_mean_eval_res_tq,Results_Decisions ] = Bayes_processFv2( Data_Sets22,Label,Extra_Apps,Extra_Labels,best_params,Game_G,Brows)
%UNTITLED5 Summary of this function goes here
%   Detailed explanation goes here
tic
Label2 = Label;
for ppq = 1: length(Extra_Labels)
        Label2(Label2 == Extra_Labels(ppq)) = [] ;
end
% New_Labels = sort(horzcat(Label2,Extra_Labels));
New_Labels = Label;
parfor c = 1 : length(Data_Sets22)    
% [ group_vote_counts_test,new_test_data_balanced_10,mean_mean_acc,mean_std_acc ,yfit_unknown_test,yfit_known_test,~] = Bayes_process_inner_first(c,Data_Sets22,Label,Extra_Apps,Extra_Labels);
[ group_vote_counts_test,new_test_data_balanced_10,mean_mean_acc,mean_std_acc ,yfit_unknown_test,yfit_known_test,~] = Bayes_process_inner_secondversion(c,Data_Sets22,Label,Extra_Apps,Extra_Labels,Game_G,Brows,best_params.minLS,best_params.Predictors);
%% Bayes
        for cc = 1 : length(Label)% length(New_Labels)
            for i = 1 : length(Label)% length(New_Labels)
                   temp = group_vote_counts_test{cc}(new_test_data_balanced_10(:,32) == New_Labels(i),:);
                   if(~isempty(temp))
                     if(New_Labels(i) == New_Labels(cc))
                        temp_yfit = yfit_unknown_test{cc};
                        tteemmpp1 = new_test_data_balanced_10(new_test_data_balanced_10(:,32) == New_Labels(cc),:);
                     else
                         tteemmpp1 = new_test_data_balanced_10(new_test_data_balanced_10(:,32) ~= New_Labels(cc),:);
                         tteemmpp2 = (tteemmpp1(:,32) == New_Labels(i));
                         temp_yfit = yfit_known_test{cc}(tteemmpp2);
                     end
                     temp = horzcat(temp,tteemmpp1(tteemmpp1(:,32) == New_Labels(i),31));
                     temp = horzcat(temp,temp_yfit);
                     unique_flows = unique(temp(:,length(New_Labels) + 1 ));
                     f_special = @(x) sum(temp(:,length(New_Labels) + 1) == x);
        ...
        %                      oo = 0 ;
        %                  end
                         unique_flow = temp(temp(:,length(New_Labels) + 1) == unique_flows_stats_eff(j),:);
                         w = best_params.w;% ;
                         ...
                         temp_decision = zeros(unique_flows_counts(j),length(New_Labels));
                         temp_dzscore= zeros(unique_flows_counts(j),length(New_Labels));
                         temp_pvalues = zeros(unique_flows_counts(j),length(New_Labels));
                         for k = 1: unique_flows_counts(j)
                            % add the logic here for baysian risk 
                            temp_dzscore(k,:) = (unique_flow(k,1:length(New_Labels)) - mean_mean_acc) ./ mean_std_acc;
                            temp_pvalues(k,:) = 2 .* normcdf(-abs(temp_dzscore(k,:)),0,1);
%                             temp_decision(k,:) =  (unique_flow(k,1:length(New_Labels)) >= (mean_mean_acc - mean_std_acc));
                                ...
                                if(k ~= 1)
                                    ...
                                    % start future 
                                     if(k < unique_flows_counts(j))
                                        ...
                                        temp_decision_future = zeros(temp_w_future-k,length(New_Labels));
                                        temp_dzscore_fututre= zeros(unique_flows_counts(j),length(New_Labels));
                                        temp_pvalues_fututre = zeros(unique_flows_counts(j),length(New_Labels));
                                        counter = 1;
                                        for l = k+1  : temp_w_future
                                            temp_dzscore_fututre(counter,:) = (unique_flow(l,1:length(New_Labels)) - mean_mean_acc) ./ mean_std_acc;
                                            temp_pvalues_fututre(counter,:) = 2 .* normcdf(-abs(temp_dzscore(l,:)),0,1);
%                                             temp_decision_future(counter,:) = (unique_flow(l,1:length(New_Labels)) >= (mean_mean_acc - mean_std_acc));
...
for c = 1 : length(Data_Sets22)
   for i = 1 : length(Label)%length(New_Labels) 
    for j = 1 : length(Label)%length(New_Labels)
...
for c = 1 : length(Data_Sets22)
    for i = 1 : length(Label)%length(New_Labels)
        temp_mean(c,:)  =mean(eval_res{c}{1}([1:length(Label)] ~= i,:)); %length(New_Labels)
%     mean_mean_fpr(i) = temp_mean(i);
    end
end
   mean_mean_temp_mean = mean(temp_mean);
   mean_mean_fpr = mean (mean_mean_temp_mean);
